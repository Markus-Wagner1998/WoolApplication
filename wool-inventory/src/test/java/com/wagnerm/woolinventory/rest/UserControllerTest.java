...
class UserControllerTest {
    ...
    @Test
    void getUser() {
        ResponseEntity<UserDTO> res = testRestTemplate.exchange(
    ...
    void updateUser() {
        ResponseEntity<UserDTO> res = testRestTemplate.exchange(
                "http://localhost:" + port + "/api/user", HttpMethod.PUT, new HttpEntity<>(User.builder().id(
                        testUser.getId()).email("test@test.de").firstName("Markus2").lastName("Wagner2").password(
                        "test").build(), getAuthHeader()),
                UserDTO.class
        );
        assertThat(res.getStatusCode().is2xxSuccessful()).isTrue();
        assertThat(res.hasBody()).isTrue();
    ...
    void updateUserOtherUserFails() {
        ResponseEntity<User> res = testRestTemplate.exchange(
                "http://localhost:" + port + "/api/user", HttpMethod.PUT, new HttpEntity<>(User.builder().id(
                        testUser.getId()).email("other@user.de").firstName("Markus2").lastName("Wagner2").password(
                        "test").build(), getAuthHeader()),
                User.class
        );
